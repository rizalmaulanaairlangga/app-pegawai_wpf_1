    ...
    <nav class="flex-1 overflow-y-auto mt-4 transition-all duration-500 ease-out">
      ...
      <ul class="px-3 space-y-2 transition-all duration-500 ease-out">
        ...
        @elseif($userRole === 'staff')
          @foreach ([
            ['url' => '/dashboard', 'icon' => 'M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5m.75-9 3-3 2.148 2.148A12.061 12.061 0 0 1 16.5 7.605', 'label' => 'Dashboard'],
            ['url' => '/myprofile', 'icon' => 'M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z', 'label' => 'My Profile'],
            ['url' => '/myattendance', 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5', 'label' => 'My Attendance'],
            ['url' => '/my-salaries', 'icon' => 'M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z', 'label' => 'My Salary'],
          ] as $menu)
            <li class="transition-all duration-500 ease-out transform"
                :class="($store.sidebar.open || $store.sidebar.pinned) ? 'translate-x-0' : 'translate-x-1'">
              <a href="{{ url($menu['url']) }}" 
                 class="flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-500 ease-out transform hover:scale-[1.02] active:scale-[0.98] group {{ active($menu['url'].'*') }}"
                 :class="request()->is($menu['url'].'*') ? 'bg-green-50 text-green-600 border-r-2 border-green-600' : 'text-gray-700 hover:bg-green-50 hover:text-green-600'">
                <span class="w-6 flex justify-center transition-all duration-500 ease-out transform"
                      :class="($store.sidebar.open || $store.sidebar.pinned) ? 'scale-100' : 'scale-110'">
                  <svg xmlns="http://www.w3.org/2000/svg" 
                       class="w-5 h-5 transition-all duration-500 ease-out group-hover:scale-110" 
        ...
        @foreach ([
          ['url' => 'settings', 'icon' => 'M4.5 12a7.5 7.5 0 0015 0m-15 0a7.5 7.5 0 1115 0m-15 0H3m16.5 0H21m-1.5 0H12m-8.457 3.077l1.41-.513m14.095-5.13l1.41-.513M5.106 17.785l1.15-.964m11.49-9.642l1.149-.964M7.501 19.795l.75-1.3m7.5-12.99l.75-1.3m-6.063 16.658l.26-1.477m2.605-14.772l.26-1.477m0 17.726l-.26-1.477M10.698 4.614l-.26-1.477M16.5 19.794l-.75-1.299M7.5 4.205L12 12m6.894 5.785l-1.149-.964M6.256 7.178l-1.15-.964m15.352 8.864l-1.41-.513M4.954 9.435l-1.41-.514M12.002 12a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z', 'label' => 'Settings'],
        ] as $menu)
          <li class="transition-all duration-500 ease-out transform"
              :class="($store.sidebar.open || $store.sidebar.pinned) ? 'translate-x-0' : 'translate-x-1'">
        ...
        <li class="transition-all duration-500 ease-out transform"
          ...
          <form action="{{ route('logout') }}" method="POST" class="w-full">
            @csrf
            <button type="submit" 
                    class="flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-500 ease-out transform hover:scale-[1.02] active:scale-[0.98] group w-full text-red-600 hover:bg-red-50 hover:text-red-700">
              <span class="w-6 flex justify-center transition-all duration-500 ease-out transform"
                    :class="($store.sidebar.open || $store.sidebar.pinned) ? 'scale-100' : 'scale-110'">
                <svg xmlns="http://www.w3.org/2000/svg" 
                     class="w-5 h-5 transition-all duration-500 ease-out group-hover:scale-110" 
