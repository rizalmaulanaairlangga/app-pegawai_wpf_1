@extends('layouts.app')
@section('title', 'Edit Gaji - ' . $salary->employee->nama_lengkap)

@section('content')
<div class="font-[Poppins] flex items-center justify-center">
    <div class="bg-white shadow-lg rounded-xl p-8 w-full max-w-5xl">
        <x-action-buttons type="back" />
        <h2 class="text-2xl font-semibold text-gray-800 mb-6 text-center">Edit Data Gaji Karyawan</h2>

        <form action="{{ route('salaries.update', $salary->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @csrf
            @method('PUT')
                ...
                <select id="karyawan_id" name="karyawan_id"
                    ...
                    @foreach ($employees as $employee)
                        <option value="{{ $employee->id }}" 
                            ...
                            data-gaji-pokok="{{ $employee->position->gaji_pokok ?? 0 }}">
            ...
            <div>
                ...
                <div class="flex rounded-lg shadow-sm">
                    ...
                    <input type="text" id="gaji_pokok" name="gaji_pokok"
                           value="{{ old('gaji_pokok', number_format($salary->gaji_pokok, 0, ',', '.')) }}"
            ...
            <div>
                ...
                <div class="flex rounded-lg shadow-sm">
                    ...
                    <input type="text" id="tunjangan" name="tunjangan"
                           value="{{ old('tunjangan', number_format($salary->tunjangan, 0, ',', '.')) }}"
                           class="salary-input flex-1 min-w-0 block w-full px-4 py-2 rounded-r-lg border border-l-0 border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none text-right tracking-wide"
            ...
            <div>
                ...
                <div class="flex rounded-lg shadow-sm">
                    ...
                    <input type="text" id="potongan" name="potongan"
                           value="{{ old('potongan', number_format($salary->potongan, 0, ',', '.')) }}"
                           class="salary-input flex-1 min-w-0 block w-full px-4 py-2 rounded-r-lg border border-l-0 border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none text-right tracking-wide"
                           data-field="potongan">
                </div>
                @error('potongan')
            ...
            <div>
                ...
                <div class="flex rounded-lg shadow-sm">
                    ...
                    <input type="text" id="total_gaji" name="total_gaji"
                           value="{{ old('total_gaji', number_format($salary->total_gaji, 0, ',', '.')) }}"
                           readonly
                ...
                <p class="text-sm text-gray-500 mt-1">*Total dihitung otomatis: Gaji Pokok + Tunjangan - Potongan</p>
                @error('total_gaji')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
